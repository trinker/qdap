...
#' @param person.var %% ~~Describe \code{person.var} here~~
#' @param text.var %% ~~Describe \code{text.var} here~~
#' @param additional.names %% ~~Describe \code{additional.names} here~~
#' @param warning %% ~~Describe \code{warning} here~~
...
#' function (dataframe, person.var, text.var, additional.names = NULL, 
#'     warning = FALSE) 
#' {
...
#'     ncheck <- function(x, place = 1, name.list = NAMES) x[place] %in% 
#'         name.list
...
#'         z <- noP %p% P
#'         cp <- if (sum(unlist(lapply(y, ccheck))) > 0) 
#'             which(unlist(lapply(y, ccheck)))[1] + 1
#'         else length(y) + 1
#'         cp2 <- if (sum(unlist(lapply(y, ccheck))) > 1) 
#'             which(unlist(lapply(y, ccheck)))[2] + 1
#'         else length(y) + 1
#'         ifelse(y[1] %in% c("sh", "shh", "shhh", "shhhh") & length(y) == 
#'             1, z, ifelse(dcheck(y) & length(y) == 1, z, ifelse(y[1] == 
#'             "duck" & length(y) == 1, z, ifelse(dcheck(y) & vcheck(y, 
#'             2), z, ifelse(y[1] == "let's" & ((vcheck(y, 2) & 
#'             !y[2] %in% c("see", "think", "ponder")) | y[2] == 
#'             "not" & vcheck(y, 3)), z, ifelse(ncheck(y) & dcheck(y, 
#'             2) & vcheck(y, 3), z, ifelse(ncheck(y) & ccheck(y, 
#'             2) & dcheck(y, 3) & vcheck(y, 4), z, ifelse(vcheck(y) & 
#'             !y[1] %in% c("do", "cause", "last", "like", "come", 
#'                 "sort") & !y[1] %in% NAMES, z, ifelse(y[1] %in% 
#'             "come" & !y[2] %in% "on", z, ifelse(y[1] %in% "come" & 
#'             y[2] %in% "on" & (!ccheck(y, 3) | (y[3] %in% "now" & 
#'             !ccheck(y, 4))), z, ifelse(acheck(y) & !y[1] %in% 
#'             c("to") & !y[2] %in% c("last") & vcheck(y, 2), z, 
#'             ifelse(acheck(y) & ccheck(y, 2) & vcheck(y, 3), z, 
#'                 ifelse(y[1] == "sort" & !y[2] %in% "of", z, ifelse(ncheck(y) & 
#'                   vcheck(y, 2), z, ifelse(y[1] == "do" & !y[2] %in% 
#'                   c("i", "you", "not", "they"), z, ifelse(y[1] == 
#'                   "do" & y[2] == "not" & vcheck(y, 3), z, ifelse(ncheck(y) & 
#'                   ccheck(y, 2) & vcheck(y, 3), z, ifelse(ncheck(y) & 
#'                   acheck(y, 2) & vcheck(y, 3), z, ifelse(ncheck(y) & 
#'                   ccheck(y, 2) & acheck(y, 3) & vcheck(y, 4), 
#'                   z, ifelse(y[1] == "and" & ncheck(y, 2) & ccheck(y, 
#'                     3) & acheck(y, 4) & vcheck(y, 5), z, ifelse(y[1] == 
#'                     "and" & ncheck(y, 2) & ccheck(y, 3) & vcheck(y, 
#'                     4), z, ifelse(y[1] == "and" & ncheck(y, 2) & 
#'                     acheck(y, 3) & vcheck(y, 4), z, ifelse(y[1] == 
#'                     "and" & ncheck(y, 2) & vcheck(y, 3), z, ifelse(y[1] == 
#'                     "and" & vcheck(y, 2) & ccheck(y, 3) & vcheck(y, 
#'                     4), z, ifelse(pcheck(y) & (vcheck(y[cp:length(y)]) | 
#'                     acheck(y[cp:length(y)]) & vcheck(y[cp:length(y)], 
#'                       2)), z, ifelse(pcheck(y) & ncheck(y[cp:length(y)]) & 
#'                     (vcheck(y[cp:length(y)], 2) | ncheck(y[cp:length(y)]) & 
#'                       acheck(y[cp:length(y)], 2) & vcheck(y[cp:length(y)], 
#'                       3)), z, ifelse(pcheck(y) & ncheck(y[cp:length(y)]) & 
#'                     ccheck(y[cp:length(y)], 2) & (vcheck(y[cp:length(y)], 
#'                     3) | ncheck(y[cp:length(y)]) & ccheck(y[cp:length(y)], 
#'                     2) & acheck(y[cp:length(y)], 3) & vcheck(y[cp:length(y)], 
#'                     4)), z, ifelse(acheck(y) & ccheck(y, 2) & 
#'                     pcheck(y, 3) & vcheck(y[cp2:length(y)]), 
#'                     z, ifelse(ncheck(y) & pcheck(y, 2) & (vcheck(y[cp:length(y)]) | 
#'                       acheck(y[cp:length(y)]) & vcheck(y[cp:length(y)], 
#'                         2)), z, ifelse(ncheck(y) & ccheck(y, 
#'                       2) & pcheck(y, 3) & (vcheck(y[cp2:length(y)]) | 
#'                       acheck(y[cp2:length(y)]) & vcheck(y[cp2:length(y)], 
#'                         2)), z, x))))))))))))))))))))))))))))))
#'     }
#'     WARN <- function(x) {
#'         SL <- nchar(x)
...
#'         y <- gsub(" +", " ", y)
#'         y <- tolower(breaker(y))
#'         a <- ifelse(ncheck(y) & dcheck(y, 2) & vcheck(y, 3), 
#'             TRUE, FALSE)
