...
#'         grouping.var
#'     }
#'     DF <- data.frame(text = as.character(text.var), group = as.factor(grouping.var), 
...
#'     switch(units, words = ner <- function(x) length(unblanker(words(strip(x)))), 
#'         characters = ner <- function(x) nchar(gsub(" ", "", x)), 
#'         syllables = ner <- function(x) syllable.sum(x), sentences = {
#'             ner <- function(x) {
...
#'             helper(ans, res.col = names(ans)[1], start.col = "start", 
#'                 end.col = "end", res.colors = plot.colors, xlab = units, 
#'                 box.color = box.color, title = paste("Speech Duration (", 
#'                   units, ")", sep = ""), y2 = y2)
...
#'     }
#'     else {
#'         ans[, 1:(length(g) + 1)] <- lapply(ans[, 1:(length(g) + 
#'             1)], as.factor)
#'     }
#'     la <- length(ans)
