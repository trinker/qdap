...
#' function (text.var, na.omit = FALSE, digits = 2, progress.bar = TRUE, 
#'     gc.rate = 10) 
#' {
...
#'         if (Sys.info()[["sysname"]] == "Windows" & progress.bar != 
#'             "text") {
...
#'                 if (i%%gc.rate == 0) 
#'                   gc()
#'                 setWinProgressBar(pb, i, title = paste(round(i/ntv * 
#'                   100, 0), "% done"))
#'                 return(x)
#'             })
#'             close(pb)
...
#'     }
#'     names(m) <- NULL
#'     poser <- function(x) sub("^.*/([^ ]+).*$", "\1", unlist(strsplit(x, 
#'         " ")))
#'     o <- lapply(m, poser)
...
#'     G5 <- sapply(data.frame(G4, check.names = FALSE), function(x) round(x/m$word.count, 
#'         digits = digits))
