...
`r FT(orange, 5, text="&diams;")` **Dictionary/Look Up Examples** `r FT(orange, 5, text="&diams;")`
...
Many qdap functions break sentences up into words based on the spaces between words.  Often the researcher will want to keep a group of words as a single unit.  The `r FUN("space_fill")` allows the researcher to replace spaces between selected phrases with `r CN("~~")`.  By defualt `r CN("~~")` is recognized by many qdap functions as a space separator.
