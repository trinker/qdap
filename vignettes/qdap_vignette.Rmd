...
1. `r HR("#wordcsv", "Coding Words - The .csv Approach")` - How to template, code, read in the data
2. `r HR("#wordtrans", "Coding Words - The Transcript Approach")` - How to template, code, read in the data
3. `r HR("#timespan", "Coding Time Spans")` - How to template, code, read in the data
...
`r FT(orange, 5, text="&diams;")` **Coding Template** - *Coding Words (csv approach)*`r FT(orange, 5, text="&diams;")`

```{r eval= FALSE}
codes <- qcv(dc, sf, wes, pol, rejk, lk, azx, mmm)
X <- cm_df.temp(DATA, "state", codes, csv=TRUE)
head(X, 10)
```
