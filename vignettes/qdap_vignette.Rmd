...
Many functions in qdap utilize the `r FUN("paste2")` function, which pastes multiple columns/lists of vectors.  `r FUN("paste2")` differs from base R's `r HR2("http://127.0.0.1:16084/library/base/html/paste.html", "paste")` function in that `r FUN("paste2")` can paste unspecified columns or a list of vectors together.  The `r FUN("colsplit2df")` and `r FUN("lcolsplit2df")` are useful because they can split the output from qdap functions that contain dataframes with pasted columns.
...
`r FT(orange, 5, text="&diams;")` **`r FUN("colsplit2df")`**: *Splitting Columns in Dataframes*`r FT(orange, 5, text="&diams;")`
...
<form action="http://trinker.github.io/qdap_dev/bag.o.words.html" target="_blank">
    <input type="submit" value="bag.o.words"><input type="submit" value="breaker"><input type="submit" value="word.split"> - `r HR("#bag", "Bag of Words")`
</form>
...
<TABLE>
    <TR> <TD align="right"><font face="courier"><b>`r HR("http://trinker.github.io/qdap_dev/bag.o.words.html", "bag.o.words")` </b></font></TD> <TD align="right">Reduces a text column to a <b>single</b> vector bag of words.</TD> </TR>
    <TR> <TD align="right"><font face="courier"><b>`r HR("http://trinker.github.io/qdap_dev/bag.o.words.html", "breaker")`</b></font></TD> <TD align="right"> Reduces a text column to a <b>single</b> vector bag of words and qdap recognized end marks.</TD> </TR>
    <TR> <TD align="right"><font face="courier"><b>`r HR("http://trinker.github.io/qdap_dev/bag.o.words.html", "word.split")`</b></font></TD> <TD align="right"> Reduces a text column to a <b>list</b> of vectors of bag of words and qdap recognized end marks (i.e., ".", "!", "?", "*", "-").</TD> </TR>
</TABLE>
...
x <- search_repo(bag.o.words, breaker, word.split)
...
It is often useful and more efficient to start with a preset vector of words and eliminate or `r FUN("exclude")` the words you do not wish to include.  Examples could range from excluding an individual(s) from a column of participant names or excluding a few select word(s) from a pre defined qdap word list.  THis is particlarly useful for passsing terms or stopwords to word counting functions like `r FUN("termco")` or `r FUN("trans.cloud")`.
...
<form action="http://trinker.github.io/qdap_dev/cm_code.blank.html" target="_blank">
    <input type="submit" value="cm_code.blank"> - `r HR("#code.blank", "Blank Code Transformation")`
</form>
...
The csv approach utilizes `r FUN("cm_df.temp")` and `r FUN("cm_df2long")` functions.  To utilize the csv template approach simply supply the dataframe, specify the text variable and provide a list of anticipated codes.  
...
The Transcript/List approach utilizes `r FUN("cm_df.transcript")`,  `r FUN("cm_range.temp")` and `r FUN("cm_range2long")` functions.  To use the transcript template simply supply the dataframe, specify the text variable and provide a list of anticipated codes.  
...
This format is not particularly useful.  The data can be reshaped to long format with durations via `r FUN("cm_range2long")`:
...
The Time Span approach utilizes the `r FUN("cm_time.temp")` and `r FUN("cm_time2long")` functions.  To generate the timespan template approach simply supply the list of anticipated codes and a start/end time.  
...
After coding the data (see the `r HR2("http://www.youtube.com/watch?v=XC-RXeY63bM&feature=youtu.be", "YouTube video")` the data can be read back in with `r HR2("http://stat.ethz.ch/R-manual/R-devel/library/base/html/source.html", "source")`.  Be sure to assign list to an object (e.g., `dat <- list()`).  Also be aware that the end times must be greater than 0.
...
This format is not particularly useful.  The data can be reshaped to long format with durations via `r FUN("cm_time2long")`:
