...
\noindent Here is a visual overview of the textbf{qdap} functions used to convert between data structures.  Many of these conversion could be achieved via the \textbf{tm} package as well.

\begin{figure}[h!]
  \centering
    \includegraphics[width=5in]{imgs/data_convert.png}
\end{figure}
