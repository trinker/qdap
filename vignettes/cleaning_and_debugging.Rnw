...
\begin{sing_enum}
  \item \emph{Check text} for potential problems
  \item Perform recommended \emph{cleaning procedures}
  \item \emph{Recheck text} for potential problems
  \item Run \emph{analysis}
  \item \emph{Debugging} to isolate errors when they occur
\end{sing_enum}
...
While the \texttt{check\_text} is useful for catching many text problems it can not detect all problems a user may encounter.  Some functions may provide useful error/warning message that help the user isolate the source of the problem.  At other times the source is unknown.
...
In halving the user experiences an error and recursively divides the data set into halves until the source of the error is found.  With each pass the non-offending portion and half of the offending data is used to test smaller and smaller pieces of the data for the location of the error.  Here is an example where I walk you through the entire process.

\noindent First we'll create an error generating function to use with the built in \textbf{qdap} data set \texttt{DATA}:
