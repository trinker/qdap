...
* `gantt_plot` did not handle `fill.vars`, particlarly if the fill was nested 
  withing the `grouping.vars`.  This behavior has been fixed with corresponding 
  examples added.
...
* `hash_look` (and `%ha%`) a counterpart to `hash` added to allow quick access 
  ...
  same hash table, whereas lookup is intended fo a single external (non 
  function) use which is more conveinent though could be slower.
...
* `polarity` utilizes a new, unbounded algorith based on weighting to determine 
  polarity.
